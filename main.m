...
Vp = [0, 3 * VTp, 0];                               % Ions vel [cm / s] (vectro size of 3!!!)
...
fgFinal = GenereteInitialDistribution(ng, VTp * sqrt(mp / mg), vGrid, Nv);
...
figure(7)
plot(time, sqrt(2) * V(:, 1), 'r-', 'LineWidth', 2);
grid on;
title('Gas v_{\perp}');
xlabel('time [s]');
ylabel('V [cm / s]');
...
figure(8)
plot(time, sqrt(2) * V(:, 3), 'r-', 'LineWidth', 2);
grid on;
title('Gas v_{\perp}');
xlabel('time [s]');
ylabel('V [cm / s]');
...
% 
% figure(7)
% Ns = linspace(0, Ntotal, Ntotal);
% plot(Ns, abs(nuSink - nuSinkOld) ./ nuSinkOld * 100);
% grid on;
% title('full nuSink and nuSink inside grid');
% xlabel('N');
% ylabel('diff');
